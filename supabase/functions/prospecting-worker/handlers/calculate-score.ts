...
export async function scoreLead({ db, lead, campaign, job }: WorkerContext) {
  ...
  const digital = {
    ...lead.digital,
    auditFailureCode: lead.audit_failure_code ?? lead.digital?.auditFailureCode ?? null,
  }
  const input = {
  ...
  const scorePhase =
    (job.payload as any)?.scorePhase ??
    (lead.score_status === 'provisional' ? 'final' : 'provisional')
  return { input, rules, score: calculateScore(input, rules), scorePhase }
}
